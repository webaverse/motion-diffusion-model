...
# Globals
male_model_path = './fbx/SMPL_m_unityDoubleBlends_lbs_10_scale5_207_v1.0.0.fbx'
female_model_path = './fbx/SMPL_f_unityDoubleBlends_lbs_10_scale5_207_v1.0.0.fbx'
...
def process_poses(
    ...
    print("--------------------------------", bpy.data.armatures[0])
    for x in bpy.data.armatures[-1].edit_bones:
        print("-----x:", x)
    pelvis_position = Vector(bpy.data.armatures[-1].edit_bones[bone_name_from_index[0]].head)
...
def outputFbx(path, outPath, _gender = "male", startOrigin = 1):
    try:
        input_path = path
        output_path = outPath
